...
const run = async() => {
    ...
    try{
        const appointmentCollection= client.db('doctorsChamber').collection('appointmentCollection')
        const bookingCollection= client.db('doctorsChamber').collection('bookingCollection')
        const usersCollection= client.db('doctorsChamber').collection('usersCollection')
    ...
    app.get('/appointments',async(req, res)=> {
        ...
        const query= {}
        const appointmentsOptions= await appointmentCollection.find(query).toArray()
        const bookingQuery= {appintmentDate: date}
        const alreadyBooked= await bookingCollection.find(bookingQuery).toArray()
        // code carefully 
        appointmentsOptions.forEach(option => {
            const optionBooked= alreadyBooked.filter(booked=> booked.servicesName === option.name) 
